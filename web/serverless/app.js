...
//   }
  const { shop, hmac, host, timestamp,code } = event.queryStringParameters
   ...
   try {
          ...
          const token = await accessToken(process.env.SHOPIFY_API_KEY,process.env.SHOPIFY_API_SECRET,shop,code);
          console.log("shopify Token ============",token);
        
            let url ="https://" + shop + "/admin/api/" + process.env.SHOPIFY_API_VERSION + "/shop.json";
          const responseShopData = await getShopDetails(shop,url,token.access_token);
  ...
  console.log("===================================",responseShop)
          ...
          console.log("response data", responseData);
              ...
              const registerShopWebhook=await postApiRest(shop,
              token.access_token,
              {
              webhook: {
          ...
          const registerOrderWebhook=await postApiRest(shop,
              token.access_token,
              {
              webhook: {
                topic: "orders/create",
         ...
         console.log("webhook order",JSON.stringify(registerOrderWebhook));
          ...
          // console.log("redirect url",red_url)
           ...
           response =buildResponse(302,"https://fbtu4zlun9.execute-api.us-east-1.amazonaws.com/api/paymentcreate?shop="+shop);
...
async function embedAppUrl(shop) {
           return process.env.HOST +'/?SHOPIFY_API_KEY=' + process.env.SHOPIFY_API_KEY +'&shop='+shop;
    }
...
async function postApiRest (shop,access_token,data){
   try {
        const response = await axios({
          url:"https://"+shop+"/admin/api/"+process.env.SHOPIFY_API_VERSION+"/webhooks.json",
          method: "POST",
          headers: {
            "X-Shopify-Access-Token":access_token,
            "Accept-Encoding": "gzip,deflate,compress"
...
//     const hmac = event.queryStringParameters.hmac;
//     const data = event.queryStringParameters
//     console.log("hmac",data,SHOPIFY_API_SECRET)
...
//       return {
//         statusCode: 400,
//         body: "Missing data or hmac in the request.",
//       };
//     }
...
//     // Calculate the HMAC locally using the Shopify API Secret
//     const digest = crypto
//       .createHmac("sha256", SHOPIFY_API_SECRET)
//       .update(message)
//       .digest("hex");
